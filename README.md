...
Unity mechanics true for any Unity repo. The skills above pull them in through the `unity` router; your repo's own facts live in its Unity config (`docs/agents/unity.md`), written by `/setup-matt-pocock-skills`.
...
- **[unity](./skills/unity/unity/SKILL.md)**: Unity knowledge entry point. Detects a Unity repo, reads its Unity config, routes a need to the Unity skills that answer it, and finds the Unity docs page for the repo's Unity version.
...
- **[unity-assemblies](./skills/unity/unity-assemblies/SKILL.md)**: Unity assembly and compilation rules. Keeps code in the right assembly, the Editor/runtime split and asmdef GUID references intact, proposes asmdef changes rather than making them, and maps assembly names and GUIDs with a script.
- **[unity-serialization](./skills/unity/unity-serialization/SKILL.md)**: Unity serialization rules. Keeps `.meta` GUIDs and serialized values intact through renames, moves and deletes, scopes the reserialize that follows a rename, edits scene and prefab text surgically, reviews big asset diffs, and resolves merge conflicts in Unity assets by file type.
- **[unity-testing](./skills/unity/unity-testing/SKILL.md)**: Unity test rules. Picks EditMode or PlayMode, keeps MonoBehaviours humble so logic tests in EditMode, avoids the engine's test traps, sets up test assemblies, and chooses which tests run and how often.
- **[unity-code-lifecycle](./skills/unity/unity-code-lifecycle/SKILL.md)**: Unity code lifecycle rules. Gives every static, singleton and static event a reset path that holds with domain reload off, picks the lifecycle API by Unity version and the module's existing pattern, and flags load-code hazards.
- **[unity-prototyping](./skills/unity/unity-prototyping/SKILL.md)**: Unity prototyping rules. Builds a throwaway prototype in the engine by default, keeps it in its own folder and editor-only asmdef out of production code and player builds, runs it from one menu item with a live variant switcher, and removes it only after a GUID grep.
- **[unity-debugging](./skills/unity/unity-debugging/SKILL.md)**: Unity debugging mechanics. Drives Play mode as one scripted loop that goes red on the bug (twice when domain reload is off, to catch second-Play bugs), uses `eval` and `run_script` as the REPL, maps a symptom to the Unity skill for its class, reads Player logs from Development Builds, and measures performance against a baseline.
