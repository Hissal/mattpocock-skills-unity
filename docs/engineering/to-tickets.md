...
In a Unity repo it also asks the [unity](https://github.com/Hissal/mattpocock-skills-unity/blob/main/skills/unity/unity/SKILL.md) router how to slice Unity work: tickets that share a scene or prefab block each other, serialized renames split into expand then contract, and work only a human can check is labelled `ready-for-human`.
