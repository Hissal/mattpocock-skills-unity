...
In a Unity repo the Standards sub-agent also asks the [unity](https://github.com/Hissal/mattpocock-skills-unity/blob/main/skills/unity/unity/SKILL.md) router for the rules on the diff's serialized types and assets, asmdefs, defines and the Editor/runtime split, statics and load code, and tests, flagging a Unity finding as hard only on data loss or a broken build.
