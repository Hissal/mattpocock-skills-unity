...
- **Unverified race**: Unity deletes a `.meta` whose asset is missing when it refreshes. With a GUI editor open and Auto Refresh on, a refresh between writing the `.meta` and the `.asmdef` would delete the `.meta` and mint a new GUID. Mitigation adopted for the skill: write both in one command, `.meta` first, re-read the `.meta` before using its GUID anywhere else.
